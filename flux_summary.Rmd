...
abs_plot <- ggplot(dat_trt, aes(color = Treatment)) + 
  ...
  geom_line(aes(Timestamp, Flux_CO2)) +
...
ggplot(dat_trt_rel_cum_co2, aes(color = Treatment)) +
  geom_line(aes(Timestamp, Flux_CO2_cum)) +
  facet_grid(Plot~.) + 
  flood_bands + 
  ggtitle("Cumulative")
```
...
abs_plot <- ggplot(dat_trt, aes(color = Treatment)) + 
  ...
  geom_line(aes(Timestamp, Flux_CH4)) +
  geom_errorbar(aes(Timestamp,
...
ggplot(dat_trt_rel_cum_ch4, aes(color = Treatment)) +
  geom_line(aes(Timestamp, Flux_CH4_cum)) +
  facet_grid(Plot~.) + 
  flood_bands + 
  ggtitle("Cumulative")
```
...
### Potentially bad filenames

```{r badnames-8100}
dat81raw %>%
  group_by(Filename, Serial, Plot) %>% 
  summarise(Start = as.Date(min(Timestamp), na.rm = TRUE), 
            End = as.Date(max(Timestamp), na.rm = TRUE),
            .groups = "drop") %>% 
  knitr::kable()
```
...
dat81 %>% 
  ...
  select(Timestamp_rnd, Plot, Collar, Treatment, Exp_Flux, Lin_Flux) ->
  dat81_rnd

dat1 %>% 
  mutate(Timestamp_rnd = round_date(Timestamp, "hour")) %>% 
  select(Timestamp_rnd, Plot, Collar, Treatment, Flux_CO2) %>% 
  inner_join(dat81_rnd, by = c("Timestamp_rnd", "Plot", "Collar", "Treatment")) -> dat_combined

ggplot(dat_combined, aes(Flux_CO2, Exp_Flux)) + 
  ...
  coord_fixed()
ggplot(dat_combined, aes(Flux_CO2, Lin_Flux)) + 
  geom_point(aes(color = Treatment), na.rm = TRUE) + 
  geom_abline() + 
  facet_wrap(~Plot, ncol = 2) + 
  xlab("7810 flux") + 
