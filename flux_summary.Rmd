...
floods$Timestamp <- NA_POSIXct_

dat1 %>% 
  group_by(File, Plot, Treatment) %>% 
  summarise(Flux_CO2_sd = sd(Flux_CO2),
            Flux_CO2 = mean(Flux_CO2),
            Flux_CH4_sd = sd(Flux_CH4),
            Flux_CH4 = mean(Flux_CH4),
...
flood_coords <- coord_cartesian(xlim = c(min(floods$Start) - 24 * 60 * 60 * 7,
                                         max(floods$End) + 24 * 60 * 60 * 7))
